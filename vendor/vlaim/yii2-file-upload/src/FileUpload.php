...
class FileUpload
{
    ...
    public function uploadFromString($filename, $msg)
    {
        switch ($this->_storage) {
        case self::S_LOCAL: $this->path = $this->uploadFromStringToLocalFS($filename, $msg);
            break;
        case self::S_S3: $this->path = $this->uploadFromStringToAmazonS3($filename, $msg);
            break;
        }
    ...
    private function uploadFromFileToLocalFS($file)
    {
        ...
        if (!file_exists($path)) {
            //throw new FileUploadException($this->_fsPath." ".$path. " ".getcwd());
    ...
    private function uploadFromStringToLocalFS($fileName, $msg)
    {
        $path = $this->getUploadFolder();
        ...
        if (file_put_contents($this->_fsPath.$path, $msg)) {
            return $this->_fsUrl.$path;
        }
    ...
    private function uploadFromStringToAmazonS3($filename, $msg)
    {
        $this->checkRequiredAWSParams();
